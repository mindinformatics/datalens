...
exp=read.csv("~/Dropbox (Partners HealthCare)/CATS-MIND/DataLENS_Paper/results-ca.tsv", sep="\t", header=T)
all = exp[match(id,exp$GeneSymbol), c("ROSMAP_PFC_FPKM_CpDxLow_AD_NCI_logFC","ROSMAP_PFC_FPKM_CpDxLow_AD_NCI_adj.P.Val","IGAP_1_Pvalue","MayoEGWAS_eQTL_TCX_AD_Pvalue")]
all[is.na(all)] <- 0
...
dat3$group[dat3$log10_exp > 2]=1
dat3$group[dat3$log10_exp < 2]=2
