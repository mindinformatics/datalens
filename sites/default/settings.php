...
$databases = array (
  ...
  array (
    'default' =>
    array (
      'database' => 'db27086_jad',
      'username' => 'u27086_jad',
      'password' => 'drupal7j4d',
      'host' => 'mysql09-int.cp.hostnet.nl',
      'port' => '',
      'driver' => 'mysql',
      'prefix' => '',
    ),
 ...
 *
 */
$drupal_hash_salt = 'l7vhOIZzzuh7J50fq6ts2PhVWaQvRcwHtUBqhGJWwrQ';
...
// Include local settings.php if it exists. This avoids having changes in
// settings.php which is checked in git re Pantheon's recommendations.
if (file_exists('sites/default/local.settings.php')) {
  include 'sites/default/local.settings.php';
}
...
if (defined('PANTHEON_ENVIRONMENT') && PANTHEON_ENVIRONMENT === 'live') {
  // Settings on live environment for all requests (web, drush and cron).
  $conf['reroute_email_enable'] = FALSE;
}
elseif (isset($_SERVER['PANTHEON_ENVIRONMENT']) && $_SERVER['PANTHEON_ENVIRONMENT'] === 'live') {
  // Settings on live environment for web requests only (excludes drush and cron).
}
else {
  // Settings on dev/test servers.
  // reroute_email settings.
  $conf['reroute_email_enable'] = TRUE;
  $conf['reroute_email_address'] = 'sciencecollaborationframework@gmail.com';
